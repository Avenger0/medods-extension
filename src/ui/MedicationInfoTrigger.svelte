<!-- src/ui/MedicationInfoTrigger.svelte -->
<script>
    ...
    const SHOW_DELAY = 200; // мс задержки перед показом
    const HIDE_DELAY = 300; // мс задержки перед скрытием
    
    onMount(() => {
        // Добавляем обработчики событий для документа, чтобы скрывать тултип при клике вне
...
<span class="info-trigger-container">
    <span 
        class="info-trigger"
    ...
    {#if showTooltip}
        <div 
            ...
            class="tooltip-wrapper {tooltipPosition}" 
